...
48~46쌍 중 성공한 공격이 각 1건뿐이라 "knockout이 억제했다"는 결과 자체는 정직하지만
표본이 너무 얇아 통계적으로 거의 의미가 없다. 지금까지 나온 모든 정황(P7 랜덤 기준선,
jaccard 유의성, Track B의 1건)이 같은 방향을 가리키긴 하지만, 신뢰 구간을 논할 수 있는
수준의 표본은 아직 없다.
...
- 7B(4bit): tool은 부르지만 인자를 잘못 타이핑(`bill-december-22023.txt` 같은 오타)하거나
  ...
  case의 정보는 못 씀).
- 4.4절에서 확인했듯 공격 문구를 바꿔도 이 병목은 안 풀림 — **모델 자체의 tool-calling
  실행 신뢰도를 올리는 것(더 큰/똑똑한 모델)이 사실상 유일하게 남은 손잡이**로 보이는데,
  현재 하드웨어(16GB GPU)에서 7B-4bit이 사실상 실용적 한계라 다음 단계(8-bit/오프로딩
  등)는 속도를 크게 희생해야 함.
...
안 고쳐졌다(배치 분리로 우회만 함). 7B 평가 중 travel suite에서 CUDA OOM 2건 발생(스킵
처리는 되지만 그 case의 정보는 손실).
...
1. **표본 확대**: 지금 표본(suite당 12쌍)은 신뢰도 있는 결론을 내기엔 부족 — 특히
   travel처럼 utility 0%인 suite는 배분 비중을 낮추고, 신호가 나오는 suite(banking/
   slack)에 더 배분.
2. **모델 크기**: 하드웨어 제약상 8-bit/오프로딩 등 타협을 감수하고 더 큰 모델을 시도할지
   결정 필요.
3. **최종 head 집합 결정**: 세 소스 단독(특히 3소스 교집합 5개 layer-0 head) vs 합집합
   vs 교집합을 Track B로 평가해 최종적으로 어떤 조합을 쓸지 결정 — 지금은 synthetic
   14개만 평가했고 InjecAgent/AgentDojo 유래 head, 합집합/교집합은 아직 안 돌려봄.
4. Top-K sweep + random-head baseline(P7 인프라)을 세 소스 전부에 동일 적용.
5. layer 0 지배(5.3) 원인 규명 — 정보 대역폭 차단 vs 명령 인식 회로.
6. 결과를 `methodology.md`/`run-guide.md`에 "Head Selection Methodology" 섹션으로 통합.
