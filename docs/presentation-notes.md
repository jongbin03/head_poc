...
> PPT 제작 참고용 문서. `methodology.md`(방법론 상세), `todo.md`(작업 이력), `.claude/memory/`
> (세션별 기록)를 종합해 발표 흐름(개요 → 방법론 → 실험 방법 → 실험 결과)에 맞게 재구성함.
> 2026-07-28 기준.

> ⚠️ **2026-07-29 자체 리뷰(`review-2026-07-29.md`) 이전에 작성된 문서다.** 아래 결론 중
> 다음 세 가지는 재검토가 필요하다:
> 1. **4.1/4.2/4.3의 완전 억제(`0.0000`)** — 합성 데이터의 공격 대상 문자열이 D_inj 안에만
>    있고 assistant prefix가 tool 호출을 강제하는 구조라, "글자를 못 보게 한" 효과가 섞여 있다.
> 2. **4.4의 "정상 기능 상승"** — InjecAgent의 read/exec 타깃이 같은 위치에서 경쟁하는
>    토큰이라 ASR 감소의 산술적 뒷면일 뿐이다.
> 3. **4.6-1의 "read head와 control head가 분리되어 있다"** — jaccard 0.29는 top-20 중 9개가
>    겹친다는 뜻이라, "internal/external끼리가 read와보다 더 겹친다"는 상대적 주장이 정확하다.
>
> 발표에 인용하기 전에 리뷰 문서를 먼저 확인할 것.
...
jaccard(합성 head, InjecAgent head) = **0.36** (완전히 같은 head 집합은 아니지만 상당수
겹치는 상태에서 세 조건 성능이 사실상 동일).
